...
//   let temp = []
//   temp = `${i.university}`
//   universities.push(temp)
// })

let universities = graduates.map((i) => {
  return {
...
zooAnimals.forEach(function (i) {
  let temp = []
  temp = `Name: ${i.animal_name}, Scientific: ${i.scientific_name}`
...
const lowerCase = zooAnimals.map((i) => {
  return {
    'Animal_name': i.animal_name.toLowerCase()
  }
})
...
let lowerPopulation = zooAnimals.filter((i) => {
